...
def generate_html_reports(analysis_data: Dict) -> Dict[str, str]:
    ...
    overall_score = analysis_data.get("ATS_score", 0) or analysis_data.get(
        "ats_score", 0
    )
    ...
    medium_priority = [
        fb
        for fb in detailed_feedback
        if fb.get("severity_level", "").lower() in ("medium", "moderate")
    ]
    ...
    unvalidated_skills = svd_raw.get("unvalidated", [])  # [{'skill',....}]
    ...
    jd_raw = analysis_data.get("jd_match_analysis") or analysis_data.get(
        "jd_comparison"
    )
