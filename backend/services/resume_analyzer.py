...
def analyze_full_resume(
    ...
    from backend.utils.file_utils import (
        get_default_grammar_result,
        get_default_location_results,
    )

    grammar_results = get_default_grammar_result()
    ...
    val_pct = round(
        (len(validated_raw) / total_skills * 100) if total_skills > 0 else 0, 1
    )
    ...
    return {
        ...
        "skills": skills,
        "matched_keywords": (
            jd_comparison_result["matched_keywords"] if jd_comparison_result else []
        ),
...
def _generate_strengths(
    ...
    """Generate a list of strengths based on the resume analysis results."""
    strengths = []

    if parsed_resume.get("experience"):
        strengths.append("Includes detailed work experience.")
    ...
    if parsed_resume.get("skills"):
        strengths.append("Clear Skills with listed technologies.")
    ...
    if len(action_verbs) >= 5:
        strengths.append(f"Good use of action verbs - {len(action_verbs)} detected.")

    validated = skill_validation.get("validated_skills", [])
    if len(validated) >= 3:
        strengths.append(
            f"Many skills validated through projects/experience - {len(validated)} skills."
        )
    ...
    if scores.get("content_score", 0) >= 20:
        strengths.append("Strong content with relevant keywords and details.")
