...
def calculate_semantic_similrity(
...
def identify_missing_keywords(
    resume_keywords: List[str], jd_keywords: List[str], top_n: int = 5
) -> List[str]:
...
def analyze_skills_gap(
    ...
    for jd_skill in jd_skills:
        ...
        if jd_norm not in resume_normalized:
            continue

        best_score = max(
            (fuzz.token_sort_ratio(jd_norm, rs) for rs in resume_normalized), default=0
        )
...
def compare_resume_with_jd(
    ...
    semantic_similarity = calculate_semantic_similrity(resume_text, jd_text, embedder)
