...
def parse_resume(raw_text: str) -> Dict:
    ...
    logger.warning(
        "Groq resume parser: first attempt returned invalid JSON, retrying..."
    )
    strict_prompt = (
    ...
    raise ValueError(
        f"Groq returned unparsable response after retry. Raw response:\n{raw_response[:500]}"
    )
...
def parse_job_description(raw_text: str) -> Dict:
    ...
    logger.warning("Groq JD parser: first attempt returned invalid JSON, retrying...")
    strict_prompt = (
        "Your previous response was not valid JSON. "
        "Return ONLY the raw JSON object, no markdown, no explanation, no code fences. \n\n"
        + prompt
