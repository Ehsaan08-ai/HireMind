    ...
    if validated:
        with st.expander(f"✅ Validated skills ({len(validated)})", expanded=False):
            for entry in validated:
                ...
                project_text = (
                    ", ".join(projects[:3]) if projects else "experience section"
                )
                sim_text = (
                ...
                st.markdown(
                    f"- **{skill}**{sim_text} — demonstrated in: {project_text}"
                )

    if unvalidated:
        with st.expander(f"⚠️ Unvalidated skills ({len(unvalidated)})", expanded=False):
            st.caption(
                "These skills are listed but not tied to a project or experience bullet."
            )
